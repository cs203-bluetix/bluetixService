...
public class TicketService {
				...
				for (Section section : sections) {
					if (section.getCategory() == ticket.getCategory()) {
						for (Session session : sessions) {
							Ticket newTicket = new Ticket(session, section, ticket.getPrice(),
									(int) section.getMaxSeat());
	...
	// return ticketRepo.findById(ticketId.getEventId(), ticketId.getVenueId(),
	// ticketId.getSectionId());
	// }
	...
	// Retrieve sessions for a specific event
	public List<Ticket> findByEventSessionId(Long eventId, Long sessionId) {
		return ticketRepo.findByEventSessionId(eventId, sessionId);
	}

	public List<Ticket> findBySessionId(Long sessionId) {
		return ticketRepo.findBySessionId(sessionId);
	}
	...
	// return savedTicket;
	// } else {
	// throw new EntityNotFoundException("Ticket not found with id: " +
	// id.toString());
	// }
	// }
